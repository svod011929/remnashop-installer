...
################################################################################
# Remnashop + Remnawave + Nginx Auto-Installer v1.0.1
...
install_docker_compose() {
    ...
    curl -fsSL "https://github.com/docker/compose/releases/download/v2.24.0/docker-compose-$(uname -s)-$(uname -m)" \
        -o /usr/local/bin/docker-compose 2>/dev/null && chmod +x /usr/local/bin/docker-compose
...
EOF
    ...
    if nginx -t >/dev/null 2>&1; then
        ...
        log_warning "Nginx config has issues, but continuing..."
    fi
}
...
ACCESS:
  URL: http://$domain
  Manage: cd $REMNASHOP_DIR && docker-compose ps
  Logs: docker-compose logs -f

NEXT STEP - GET SSL CERTIFICATE:
...
4. Update Nginx config with SSL (see below):
...
1. Edit configuration:
   nano $REMNASHOP_DIR/.env
   
2. Add Remnawave integration:
   REMNAWAVE_TOKEN=your_token
   REMNAWAVE_WEBHOOK_SECRET=your_secret
