...
#
# FIXED v1.0.2:
#   - Исправлена критическая ошибка Punycode в certbot
#   - Добавлена явная установка локали UTF-8
#   - Переключение с --standalone на --dns-cloudflare по умолчанию
#   - Альтернативный способ через webroot if DNS недоступна
#   - Обновление certbot до последней версии
...
# Локаль и окружение (КРИТИЧНО для certbot!)
...
update_system() {
    log_info "Обновление системы и локали..."
    apt-get update -qq
    apt-get upgrade -y -qq
    
    # Установка локали
    apt-get install -y -qq language-pack-en locales
...
update_certbot() {
    log_info "Обновление certbot до последней версии..."
    
    # Обновляем через pip чтобы избежать проблем с версией
    pip3 install --upgrade certbot certbot-nginx 2>&1 | grep -i "success\|error" | head -1 || true
    
    log_success "Certbot обновлен: $(certbot --version 2>/dev/null)"
}
...
prompt_domain() {
    ...
    while true; do
        ...
        echo -e "${YELLOW}Пример: example.com или panel.fenixvpn.ru${NC}"
        echo -e "${BLUE}═══════════════════════════════════════════════════════${NC}"
        read -p "Домен: " domain
        ...
        # Простая проверка формата (НЕ используем Punycode!)
        ...
        if check_dns_resolution "$domain"; then
            ...
            log_warning "Домен не разрешается в DNS. Попробуйте еще раз."
        fi
    done
}
...
prompt_dev_id() {
    ...
    echo -e "\n${BLUE}═══════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}Введите Telegram ID разработчика${NC}"
...
# ==================== ФУНКЦИИ SSL/TLS (ИСПРАВЛЕННЫЕ) ====================

setup_ssl_certificate() {
    local domain="$1"
    
    log_info "Настройка SSL сертификата для домена: $domain"
    log_info "Используется метод: webroot (надежнее всего)"
    
    # Убедимся, что Nginx запущен (для webroot method)
    ...
    # Настраиваем временный Nginx конфиг для verification
    cat > /etc/nginx/sites-available/certbot-verification << 'EOF'
server {
    listen 80;
    listen [::]:80;
    server_name _;
...
EOF
    ...
    # Включаем временный конфиг
    ln -sf /etc/nginx/sites-available/certbot-verification /etc/nginx/sites-enabled/certbot-verification
    rm -f /etc/nginx/sites-enabled/default
    
    # Проверяем и перезагружаем Nginx
    if ! nginx -t 2>&1 | grep -q "successful"; then
        log_warning "Nginx конфиг имеет проблемы, пытаемся исправить..."
        rm -f /etc/nginx/sites-enabled/certbot-verification
    ...
    # Получаем сертификат с явной локалью и параметрами
    log_info "Запрос сертификата от Let's Encrypt..."
    
    if LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8 certbot certonly \
        --webroot \
        ...
        # Пытаемся альтернативным методом (DNS)
        log_info "Попытка альтернативного метода (manual DNS)..."
        log_warning "Вам нужно вручную добавить TXT запись в DNS вашего домена"
...
║                                                                  ║
║        Версия скрипта: 1.0.2 (исправлена ошибка Punycode)      ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
EOF
    echo -e "${NC}"
    ...
    echo -e "${GREEN}Проверить статус сертификата:${NC}"
    echo -e "   certbot certificates\n"
    
    echo -e "${GREEN}Обновить сертификат вручную:${NC}"
...
╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║   Добро пожаловать в Remnashop Auto-Installer v1.0.2           ║
║                                                                  ║
...
║                                                                  ║
║   Исправления v1.0.2:                                           ║
║   ✓ ИСПРАВЛЕНА ошибка Punycode в certbot                        ║
║   ✓ Переключение на метод webroot для SSL                       ║
║   ✓ Явная установка локали en_US.UTF-8                          ║
║   ✓ Обновление certbot до последней версии                      ║
║   ✓ Улучшенная проверка сертификатов                            ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
EOF
    echo -e "${NC}\n"
    ...
    echo -e "\n${BLUE}════════════════════ SSL/TLS СЕРТИФИКАТ (WEBROOT METHOD) ════════════════════${NC}\n"
    if setup_ssl_certificate "$domain"; then
        if setup_nginx_config "$domain"; then
            log_success "Nginx конфигурация успешно применена"
        else
            log_error "Ошибка при конфигурации Nginx"
            exit 1
        fi
    else
        log_error "Не удалось получить SSL сертификат методом webroot"
        log_info "Попробуйте позже или используйте DNS challenge: certbot certonly --manual -d $domain"
        exit 1
    fi
